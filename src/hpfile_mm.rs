...
impl Segment {
    fn as_mut_file(&mut self) -> io::Result<&mut File> {
        match self {
            Segment::DiskFile(f) => Ok(f),
            Segment::MmapFile(_) => Err(io::Error::new(
    ...
    fn read_at(&self, buf: &mut [u8], offset: i64, segment_size: i64) -> io::Result<usize> {
        match self {
            Segment::DiskFile(f) => f.read_at(buf, offset as u64),
            Segment::MmapFile(f) => {
...
impl HPFile {
    ...
    fn load_file_map(
        ...
        id_list: Vec<i64>,
        largest_id: i64,
    ) -> Result<(DashMap<i64, Segment>, i64)> {
        ...
        for &id in &id_list {
            ...
            file_map.insert(id, Segment::MmapFile(mmap));
        }

        if id_list.is_empty() {
            let file_name = format!("{}/{}-{}", &dir_name, 0, segment_size);
            let file = File::create_new(file_name)?;
            file_map.insert(0, Segment::DiskFile(file));
        }
    ...
    pub fn truncate(&self, size: i64) -> io::Result<()> {
        ...
        self.file_map.insert(largest_id, Segment::DiskFile(f));
...
mod tests {
    ...
    #[test]
    fn test_pre_reader() {
        let temp_dir = TempDir::new("mm_pre_reader_test");
    ...
    #[test]
    fn test_hp_file_new() {
        let dir = TempDir::new("mm_hp_file_test");
        let buffer_size = 64;
    ...
    fn test_new_file_invalid_buffer_or_segment_size() {
        let dir = TempDir::new("mm_test_new_file_invalid_buffer_or_segment_size");
        let buffer_size = 64;
    ...
    fn test_new_file_invalid_filename() {
        let dir = TempDir::new("mm_test_new_file_invalid_filename");
    ...
    fn test_new_file_invalid_filename2() {
        let dir = TempDir::new("mm_test_new_file_invalid_filename2");
    ...
    fn test_new_file_invalid_filename3() {
        let dir = TempDir::new("mm_test_new_file_invalid_filename3");
    ...
    fn test_new_file_failed_invalid_size() {
        let dir = TempDir::new("mm_test_new_file_failed_invalid_size");
    ...
    #[test]
    fn test_hpfile() {
        let dir = TempDir::new("mm_hpfile_test_dir_6");
        let buffer_size = 64;
        let segment_size = 128;
        let hp_file = HPFile::new(buffer_size, segment_size, dir.to_string()).unwrap();
        ...
        assert_eq!(
            dir.list().join(","),
            [
                "mm_hpfile_test_dir_6/0-128",
                "mm_hpfile_test_dir_6/1-128",
                "mm_hpfile_test_dir_6/10-128",
                "mm_hpfile_test_dir_6/11-128",
                "mm_hpfile_test_dir_6/12-128",
                "mm_hpfile_test_dir_6/13-128",
                "mm_hpfile_test_dir_6/14-128",
                "mm_hpfile_test_dir_6/15-128",
                "mm_hpfile_test_dir_6/2-128",
                "mm_hpfile_test_dir_6/3-128",
                "mm_hpfile_test_dir_6/4-128",
                "mm_hpfile_test_dir_6/5-128",
                "mm_hpfile_test_dir_6/6-128",
                "mm_hpfile_test_dir_6/7-128",
                "mm_hpfile_test_dir_6/8-128",
                "mm_hpfile_test_dir_6/9-128",
            ]
            .join(",")
        );
